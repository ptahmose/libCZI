...
namespace libCZI
{
	...
	struct GeneralDocumentInfo
	{
		...
		GeneralDocumentInfo() : name_valid(false), title_valid(false), userName_valid(false), description_valid(false), comment_valid(false), keywords_valid(false), rating_valid(false), rating(0), creationDateTime_valid(false) {}
	...
	public:
		...
		/// \param [in,out] p If non-null, a pointer to a integer to store the result in.
		/// \returns True if it succeeds, false if it fails.
